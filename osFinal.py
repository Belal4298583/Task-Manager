...
tasks = [
    {
        ...
        'period': 50,
        'execution_time': 20,
        'resources': {'R1': 1, 'R2': 1, 'R3': 0},
        'processor': 1,
        'repeat': 4,
        'initial_execution_time': 20
    },
    {
        'id': 'T2',
        'period': 30,
        'execution_time': 10,
        'resources': {'R1': 1, 'R2': 2, 'R3': 0},
        'processor': 2,
        'repeat': 3,
        'initial_execution_time': 10
    },
    {
        'id': 'T3',
        'period': 70,
        'execution_time': 30,
        'resources': {'R1': 2, 'R2': 0, 'R3': 1},
        'processor': 1,
        'repeat': 2,
        'initial_execution_time': 30
    }
]
...
class MainThread(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            time_unit += 1
            self.gui.root.after(0, self.gui.update_status, time_unit)  # Use after to update GUI from the main thread
            time.sleep(1)
...
class GUI:
    def __init__(self, root):
        ...
        for i in range(1, 4):
            ...
            cpu_frame.grid(row=6+i, column=0, sticky=(tk.W, tk.E))
...
def main():
    ...
    processors = [ProcessorThread(f'P{i}', None) for i in range(1, 4)]
    for processor in processors:
        processor.processors = processors
    ...
    gui.set_processors({f'P{i}': processor for i, processor in enumerate(processors, start=1)})

    distribute_tasks(algorithm='EDF')
